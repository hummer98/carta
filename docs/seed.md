...
4. **[解決]** 既存プロジェクト CLAUDE.md の「言語ルール」「コーディング規約」セクションを憲法側に巻き取るリファクタガイド → 次回リリースで `/carta:migrate` を導入し、**マーカー外の重複記述を検出して削除候補として提示する** 形で対応（§16 参照）。書き換え提案ではなく **削除提案のみ** とし、carta 導入直後の掃除用途に位置づける。
...
`carta` を導入する前から運用していたプロジェクトの `CLAUDE.md` には、`/carta:apply` でマーカーを差し込んだ後も、マーカー外に「日本語でドキュメントを書く」「テストを必ず実行する」など **憲法と重複する記述** が残り続ける。これは drift（マーカー内は更新されたがマーカー外の旧版が残る）と矛盾指示の温床になる。

`/carta:migrate` は **マーカー外領域から、憲法本体（profiles 連結後）と意味的に重複する記述を検出し、削除候補を unified diff として提示する** 移行コマンド。carta 導入直後に **一度走らせて掃除する** 用途を想定し、定常運用ではない。

### 16.1 設計の要点（詳細は `commands/migrate.md` 参照）

- **削除のみ提案**（言い換え保持はしない）
- **マーカー内本文は 1 byte も変更しない**（書き込み後 assertion で byte 比較）
- デフォルト dry-run。`--apply` で書き込みモードに入っても最終承認プロンプトを取る
- 確信度 `HIGH` / `MEDIUM` / `LOW` のうち、デフォルトの削除候補は **HIGH のみ**。`--include-medium` で MEDIUM を昇格可能。LOW は常に参考表示のみ
- 検出は **正規化マッチング（bash + Python）** + **意味判定（このコマンドを読んでいる Claude 自身による内部評価）** のハイブリッド
- profiles は apply / diff と同じ引数規約で、未指定時は既存マーカーから引き継ぐ
- `--force` は受け取らない（マーカー内 sha 不一致時は `/carta:apply --force` で先に同期する責務分離）
- マーカー無し / 破損 / sha 不一致は誘導メッセージで停止

詳細実装仕様（コマンド本文・bash 擬似実装・エッジケース・出力フォーマット）は `commands/migrate.md` に集約する。本 seed には概要のみを残し、実装の単一ソースとしてコマンドファイルを尊重する立て付け。
